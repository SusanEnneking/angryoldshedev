...
func TestBlogGetAll(t *testing.T) {
	...
	query := `SELECT blog.id as id, body_id, auto_generated, topic, title, question_posed_to_open_ai, 
				...
				category.category as category, blog_body.body as body
				FROM blog
				JOIN category
				ON blog.category_id = category.id
				JOIN blog_body
				ON blog.body_id = blog_body.id`
	rows := sqlmock.NewRows([]string{"id", "body_id", "auto_generated", "topic", "title", "question_posed_to_open_ai", "created_date", "created_by", "is_active", "category", "body"}).
		AddRow(b.Id, b.BodyId, b.Autogenerated, b.Topic, b.Title, b.QuestionPosedToOpenAi, b.CreatedOn, b.CreatedBy, b.Active, b.Category, b.Body)
